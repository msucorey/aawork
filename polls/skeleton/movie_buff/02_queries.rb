...
def bad_years
  ...
  # Movie.where("score > 7").having("COUNT(*) < 1").group(:yr).pluck(:yr)
...
def vanity_projects
  ...
  Movie.select(:id, :title, :name).joins(:director, :castings).where("castings.ord = 1 AND castings.actor_id = movies.director_id")
...
def most_supportive
  ...
  Actor.select(:id, :name, "COUNT(*)").joins(:castings)
