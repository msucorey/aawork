...
def harrison_ford
  ...
  Movie.select(:id, :title).joins(:actors).where("castings.ord!= ? AND name = ?" , "1", 'Harrison Ford')
  # Actor.find_by_name("Harrison Ford").movies.select(:id, :title).joins(:castings).where("castings.ord != 1")
end
